...
export default function CustomerLoginPage() {
  ...
  const [busy, setBusy] = useState(false); const [resending, setResending] = useState(false); const [message, setMessage] = useState(""); const [error, setError] = useState("");

  function getSafeDestination() { const next = new URLSearchParams(window.location.search).get("next") || ""; return next.startsWith("/") && !next.startsWith("//") ? next : "/account"; }
  function getCallback() { const url = new URL("/account/login", window.location.origin); url.searchParams.set("verified", "1"); const next = new URLSearchParams(window.location.search).get("next") || ""; if (next.startsWith("/") && !next.startsWith("//")) url.searchParams.set("next", next); return url.toString(); }

  useEffect(() => {
    const params = new URLSearchParams(window.location.search); const hash = new URLSearchParams(window.location.hash.slice(1));
    const accessToken = hash.get("access_token"); const refreshToken = hash.get("refresh_token"); const hashError = hash.get("error_description");
    if (hashError) setError(hashError.replace(/\+/g, " "));
    if (accessToken) {
      localStorage.setItem("cq_access_token", accessToken); if (refreshToken) localStorage.setItem("cq_refresh_token", refreshToken);
      window.history.replaceState({}, document.title, window.location.pathname + window.location.search);
      if (params.get("verified") === "1") { window.location.replace(getSafeDestination()); return; }
  ...
  async function resend() {
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/auth/v1/resend`, { method: "POST", headers: { apikey: SUPABASE_KEY, "Content-Type": "application/json" }, body: JSON.stringify({ type: "signup", email: email.trim(), options: { emailRedirectTo: getCallback() } }) });
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "Could not resend confirmation email."); }
