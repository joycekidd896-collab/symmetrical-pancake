  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/auth/v1/recover`, {
        ...
        body: JSON.stringify({ email: email.trim(), options: { redirectTo } }),
      });
      ...
      setMessage("If that account exists, password reset instructions have been sent. Check your inbox and spam folder.");
  ...
  return (
    <main className="queen-page">
      <header className="site-header">
        ...
        <nav aria-label="Main navigation"><Link href="/coupons">Coupons</Link><Link href="/businesses">Businesses</Link><Link href="/account/login">Sign In</Link></nav>
      </header>
      <section className="auth-shell">
        <div className="section-kicker">♕ THE QUEEN'S CLUB</div><h1>Reset Your Password</h1>
        ...
        <form onSubmit={submit} className="auth-card"><label>Email address<input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" /></label><button className="queen-button primary-button" disabled={busy} type="submit">{busy ? "Sending Reset Link…" : "Send Reset Link →"}</button></form>
