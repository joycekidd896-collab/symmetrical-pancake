...
export default function CouponDetailPage() {
  ...
  const redemptionCode = `QUEEN-${coupon.id.replace("queen-", "")}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
  const handleRedeem = async () => {
    setSaving(true); setSaveError("");
    try {
      const response = await fetch("https://unpgjnlhcbtbspcehfsw.supabase.co/functions/v1/redeem-coupon", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ coupon_id: coupon.id, redemption_code: redemptionCode }) });
      if (!response.ok) throw new Error("Redemption could not be recorded");
      window.localStorage.setItem(`coupon-queen-redeemed-${coupon.id}`, "true"); setRedeemed(true);
    } catch { setSaveError("We couldn't connect to the redemption vault. Please try again."); } finally { setSaving(false); }
  };
  ...
  return (
    <main className="queen-page">
      ...
      <section className="coupon-detail-section"><Link href="/coupons" className="back-link">← Back to Deal Vault</Link><div className="coupon-detail-layout">
        ...
        <aside className="redemption-card"><div className="section-kicker">✦ REDEEM YOUR SAVINGS ✦</div>{!redeemed ? <><h2>Unlock this royal deal</h2><p>Reveal your unique redemption code when you're ready to use this offer. Your claim is securely recorded in the redemption vault.</p><button type="button" className="queen-button primary-button redeem-button" onClick={handleRedeem} disabled={saving}>{saving ? "Securing Your Deal..." : "Redeem This Deal →"}</button>{saveError && <p className="save-error" role="alert">{saveError}</p>}<p className="redemption-note">Only tap redeem when you are ready to present the offer.</p></> : <div className="redeemed-state"><div className="redeemed-check">✓</div><div className="section-kicker">DEAL UNLOCKED</div><h2>Your royal code</h2><div className="redemption-code" aria-label="Your coupon redemption code">{redemptionCode}</div><button type="button" className="copy-code" onClick={handleCopy}>{copied ? "Copied! ✓" : "Copy Code"}</button><p>Show this code to the merchant to request your savings.</p></div>}</aside>
      </div><section className="terms-card"><div className="section-kicker">♢ THE FINE PRINT</div><h2>Deal Terms</h2><ul>{coupon.terms.map((term) => <li key={term}>◆ {term}</li>)}</ul><p className="terms-warning">Sample offers are being used while the Coupon Queen marketplace is being built. Final merchant terms should be confirmed before launch.</p></section></section>
