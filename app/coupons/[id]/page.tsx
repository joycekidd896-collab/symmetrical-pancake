  ...
  return (
    <main className="queen-page">
      ...
      <section className="coupon-detail-section"><Link href="/coupons" className="back-link">← Back to Deal Vault</Link><div className="coupon-detail-layout">
        ...
        <aside className="redemption-card" aria-live="polite"><div className="section-kicker">✦ REDEEM YOUR SAVINGS ✦</div>{!redeemed ? <><h2>Unlock this royal deal</h2><p>When you're ready to use this offer, tap below to claim your unique redemption code.</p><button type="button" className="queen-button primary-button redeem-button" onClick={handleRedeem} disabled={saving}>{saving ? "Securing Your Deal..." : "Redeem This Deal →"}</button>{saveError && <p className="save-error" role="alert">{saveError}</p>}<p className="redemption-note">Only tap redeem when you are ready to present the offer.</p></> : <div className="redeemed-state"><div className="redeemed-check">✓</div><div className="section-kicker">DEAL UNLOCKED</div><h2>Your royal code</h2><div className="redemption-code" aria-label="Your coupon redemption code">{redemptionCode}</div><button type="button" className="copy-code" onClick={handleCopy}>{copied ? "Copied! ✓" : "Copy Code"}</button><p>Show this code to the merchant to request your savings.</p><div className="claimed-note">✓ This deal is saved on this device.</div></div>}</aside>
