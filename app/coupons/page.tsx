...
export default function CouponsPage() {
  ...
  const categories = useMemo(() => ["All Deals", ...Array.from(new Set(liveOffers.map((offer) => offer.category).filter(Boolean))).sort()], [liveOffers]);

  const filteredCoupons = useMemo(() => {
    const q = query.trim().toLowerCase();
    const filtered = liveOffers.filter((coupon) =>
      (activeCategory === "All Deals" || coupon.category === activeCategory) &&
      (!q || `${coupon.title} ${coupon.business} ${coupon.category} ${coupon.location} ${coupon.description}`.toLowerCase().includes(q))
    );
  ...
  const clearFilters = () => { setQuery(""); setActiveCategory("All Deals"); setSort("newest"); };

  return <main className="queen-page">
    <LiveOffers onLoad={onLiveOffers} />
    <header className="site-header"><Link href="/" className="brand-wrap" aria-label="Coupon Queen home"><div className="brand-crown">♕</div><div><div className="brand-name">COUPON QUEEN</div><div className="brand-tagline">The Crown Jewel of Savings</div></div></Link><nav aria-label="Main navigation"><Link href="/coupons" aria-current="page">Coupons</Link><Link href="/businesses">Businesses</Link></nav><div className="header-sparkle" aria-hidden="true">✦</div></header>
    <section className="benefits-section">
      ...
      <div className="coupon-toolbar"><label className="coupon-search"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search live deals, businesses, categories..." aria-label="Search live coupons" /></label><div className="coupon-filters" role="tablist">{categories.map((category) => <button key={category} type="button" role="tab" aria-selected={activeCategory === category} className={`filter-pill ${activeCategory === category ? "active" : ""}`} onClick={() => setActiveCategory(category)}>{category}</button>)}</div></div>
      <div className="coupon-results-bar"><span><strong>{filteredCoupons.length}</strong> {filteredCoupons.length === 1 ? "live deal" : "live deals"} available</span><label className="sort-control">Sort <select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort live deals"><option value="newest">Newest</option><option value="business">Business name</option><option value="discount">Discount</option></select></label>{(query || activeCategory !== "All Deals" || sort !== "newest") && <button type="button" className="clear-filters" onClick={clearFilters}>Clear filters ×</button>}</div>
      {filteredCoupons.length > 0 ? <div className="benefit-grid">{filteredCoupons.map((coupon) => <article key={coupon.id} className="benefit-card"><div className="card-icon gold-icon">🎟️</div><div className="card-number">{coupon.discount}</div><div className="coupon-category">{coupon.category}</div><h3>{coupon.title}</h3><div className="business-name">♛ <Link href={coupon.business_id ? `/businesses/${coupon.business_id}` : "/businesses"}>{coupon.business}</Link></div><p>{coupon.description}</p><div className="coupon-meta"><span>♢ {coupon.category}</span><span>📍 {coupon.location}</span></div><div className="location-note">Available at <strong>{coupon.business}</strong> · {coupon.location}</div><div className="coupon-expiry">♛ {coupon.expires}</div><Link href={`/coupons/${coupon.id}`} className="card-link">View Deal <span>→</span></Link></article>)}</div> : <div className="empty-coupon-state"><div className="card-icon gold-icon">👑</div><div className="section-kicker">THE VAULT IS READY</div><h2>{liveOffers.length ? "No matching deals" : "No live deals yet"}</h2><p>{liveOffers.length ? "Try another category or search term." : "Coupon Queen is connected to the live merchant marketplace, but there are currently no active offers. When a merchant publishes an eligible offer, it will appear here automatically."}</p>{(query || activeCategory !== "All Deals") && <button type="button" className="queen-button primary-button" onClick={clearFilters}>Show All Live Deals</button>}<Link href="/businesses/login?mode=signup" className="queen-button secondary-button">Become a Merchant →</Link></div>}
    </section>
