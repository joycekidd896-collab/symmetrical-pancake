...
export default function CouponsPage() {
  ...
  const filteredCoupons = useMemo(() => {
    ...
    return [...filtered].sort((a, b) =>
      ...
      sort === "discount" ? b.discount.localeCompare(a.discount) :
    ...
    <main className="queen-page">
      ...
      <section className="benefits-section">
        <div className="section-heading">
          ...
          <h1>Live Deals. Real Savings.</h1>
          <p>Every offer shown here comes from the live Coupon Queen marketplace. No placeholder deals are displayed.</p>
        </div>

        <div className="marketplace-strip">
          <span>● LIVE MARKETPLACE</span><span>✓ Merchant-managed offers</span><span>✓ Clear business &amp; location</span><span>✓ Secure redemption</span>
        </div>
        ...
        <div className="coupon-toolbar">
          ...
          <div className="coupon-filters" role="tablist">
            {categories.map((category) => <button key={category} type="button" role="tab" aria-selected={activeCategory === category} className={`filter-pill ${activeCategory === category ? "active" : ""}`} onClick={() => setActiveCategory(category)}>{category}</button>)}
          </div>
        </div>

        <div className="coupon-results-bar">
          <span><strong>{filteredCoupons.length}</strong> {filteredCoupons.length === 1 ? "live deal" : "live deals"} available</span>
          <label className="sort-control">Sort <select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort live deals"><option value="featured">Featured</option><option value="business">Business name</option><option value="discount">Discount</option></select></label>
          {(query || activeCategory !== "All Deals") && <button type="button" className="clear-filters" onClick={clearFilters}>Clear filters ×</button>}
        </div>

        {filteredCoupons.length > 0 ? (
          <div className="benefit-grid">
            {filteredCoupons.map((coupon) => (
              <article key={coupon.id} className={`benefit-card ${coupon.featured ? "featured-card" : ""}`}>
                ...
                <h3>{coupon.title}</h3>
                <div className="business-name">♛ <Link href={coupon.id.startsWith("live-") ? `/businesses/${coupon.id.slice(5)}` : "/businesses"}>{coupon.business}</Link></div>
                <p>{coupon.description}</p>
                <div className="coupon-meta"><span>♢ {coupon.category}</span><span>📍 {coupon.location}</span></div>
                <div className="location-note">Available at <strong>{coupon.business}</strong> · {coupon.location}</div>
                <div className="coupon-expiry">♛ {coupon.expires}</div>
                <Link href={`/coupons/${coupon.id}`} className="card-link">View Deal <span>→</span></Link>
              </article>
            ))}
          </div>
        ) : (
          <div className="empty-coupon-state">
            ...
            <div className="section-kicker">THE VAULT IS READY</div>
            <h2>No live deals yet</h2>
            <p>Coupon Queen is connected to the live merchant marketplace, but there are currently no active offers. When a merchant publishes an eligible offer, it will appear here automatically.</p>
            {(query || activeCategory !== "All Deals") && <button type="button" className="queen-button primary-button" onClick={clearFilters}>Show All Live Deals</button>}
            <Link href="/businesses/login?mode=signup" className="queen-button secondary-button">Become a Merchant →</Link>
          </div>
        )}
      </section>
