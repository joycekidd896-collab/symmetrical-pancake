...
type Merchant = { id: string; business_name: string; city: string | null; state: string | null; is_approved: boolean; is_active: boolean };
...
export default function CreateCouponPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const merchantRes = await fetch(`${api("merchants")}?owner_user_id=eq.${encodeURIComponent(user.id)}&select=id,business_name,city,state,is_approved,is_active&order=created_at.asc&limit=1`, { headers, cache: "no-store" });
        ...
        if (!currentMerchant.is_active) throw new Error("This merchant account is inactive. Contact Coupon Queen before publishing new offers.");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!merchant.is_active) throw new Error("This merchant account is inactive.");
      ...
      if (!response.ok) throw new Error(body?.message || `Could not publish the coupon (${response.status}). Please try again.`);
      setSaved(true);
      localStorage.removeItem(draftKey);
      setForm((current) => ({ ...current, title: "", discount: "", description: "", terms: "One redemption per customer.\nMerchant terms may apply.", expiresAt: "" }));
    } catch (e) {
