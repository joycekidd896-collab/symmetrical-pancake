  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "signup" && !data.access_token) {
        ...
        setMessage("Account created! Check your email to confirm your account, then sign in to continue."); setMode("login"); return;
      }
      if (!data.access_token) throw new Error("Sign-in succeeded without an access token. Please sign in again.");
      localStorage.setItem("cq_access_token", data.access_token); localStorage.setItem("cq_refresh_token", data.refresh_token || "");
